...
- Every archive has a `<archive>.spdx.json` SBOM generated by the pinned Syft
  version. Final archives are subjects of one GitHub/Sigstore SLSA provenance
  bundle, verified against the tag commit and release workflow before the draft
  is published. Keep `id-token: write` and `attestations: write` scoped to the
