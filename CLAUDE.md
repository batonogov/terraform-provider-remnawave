...
The required status checks in `.github/repository-settings/main-ruleset.json`
must demand exactly the jobs the release gate demands. The gate derives its
acceptance matrix from `compat-versions.json`, so adding a supported version
without adding its `Acceptance Tests (X)` context leaves that job
non-blocking — a pull request could merge with it red, land it on `main`, and
stall the release the gate then refuses to pass.
`scripts/test-configure-repository-security.sh` enforces the alignment.

`can_approve_pull_request_reviews` **must stay `true`** in
`.github/repository-settings/workflow-permissions.json`. GitHub bundles
"create" and "approve" behind that single toggle, and `release-please` opens
its release pull request as `github-actions[bot]`; setting it to `false` fails
the release with `GitHub Actions is not permitted to create or approve pull
requests` and no release PR is ever created. Self-approval is not a real
exposure here because `main` requires status checks that a bot approval cannot
satisfy. `default_workflow_permissions` stays `read` — every workflow that
needs more grants it per job.
